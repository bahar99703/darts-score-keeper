...
var GameType;
(function (GameType) {
    GameType[GameType["GAME_301"] = 301] = "GAME_301";
    GameType[GameType["GAME_501"] = 501] = "GAME_501";
})(GameType || (GameType = {}));
// ============================================
...
class GameState {
    constructor() {
        ...
        this.startScore = GameType.GAME_501;
        this.maxLegs = 5;
        this.currentLeg = 1;
        this.gameOver = false;
        this.legTurnHistory = [];
...
class GameLogic {
    ...
    recordTurn(points) {
        ...
        if (isBust) {
            if (this.state.lastBustPlayerId !== null &&
    ...
    removeTurn(turnNumber) {
        if (this.state.gameOver)
            return;
        const turnIndex = this.state.legTurnHistory.findIndex((t) => t.turnNumber === turnNumber);
        if (turnIndex === -1)
            return;
    ...
    recalculateScores() {
        ...
        if (winningPlayer) {
            alert(`${winningPlayer.name} reached exactly zero and wins Leg ${this.state.currentLeg}!`);
        }
    }
    endLeg(winner) {
        winner.legsWon++;
...
let gameState;
let gameLogic;
function startGame(playerNames, gameType, setSize) {
...
function renderState() {
    ...
    if (!gameDiv)
        return;
    const turnHistoryTable = gameState.legTurnHistory.length > 0
        ? `
    <h3>Turn History</h3>
